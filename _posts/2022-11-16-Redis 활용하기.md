---
layout: article
title: "게임 랭킹시스템 구현하기(Feat Redis)"
subtitle: "게임 랭킹시스템 구현하기(Feat Redis)"
...
## 플로우
<img width="852" alt="스크린샷 2022-11-17 오후 5 28 33" src="https://user-images.githubusercontent.com/99805929/202396442-bf94f17f-0bca-4283-a918-04484eab62d8.png"><br/>
<br/>
...
Postman으로 스코어를 가져오는 테스트를 진행해보니 약 3배의 속도차이가 났습니다.(Redis: 55ms, Http Request: 155ms)<br/>
<br/>
...
해결하지 못한 문제가 있는데 실시간으로 서버에서 랭킹을 받아오지 못하고, 로컬에서 플레이한 사람의 기록만 보여줄수 있습니다.<br/>
마치 문방구 오락기같은 느낌이랄까..?<br/>
구현 방식은 점수를 SET해주고 rank 순으로 뽑아내도록 구현했습니다.
...
//레이드 입장
  async raidEnter(@Body(ValidationPipe) createBossraidDto: CreateBossraidDto) {
    ...
    const getRedisLevel = await this.redis.get(`level${level}`)  //Redis에 정보가 있는지 확인
    await this.redis.set(`user${userId}EnterLevel`, level)
    if(!getRedisLevel && level <= maxLevel) { //Redis에 정보가 없다면 Request요청을 보내 받은 정보를 Redis에 저장
      await firstValueFrom(this.httpService.get('http request 주소').pipe(
        map((res) => {
...
# Reference
[Repository](https://github.com/JeongJaeHa/bossraid)
